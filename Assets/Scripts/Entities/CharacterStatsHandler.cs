...
public class CharacterStatsHandler : MonoBehaviour
{
    ...
    private void UpdateCharacterStats()
    {
        ...
        UpdateStats((a, b) => b, baseStats); // a, b 를 받아서 b를 쓰겠다.
        if (CurrentStats.attackSO != null)
        {
            CurrentStats.attackSO.target = baseStats.attackSO.target;
        }

        foreach (CharacterStats modifier in statsModifiers.OrderBy(o => o.statsChangeType)) // Linq
        {
            if (modifier.statsChangeType == StatsChangeType.Override) // 뒤에 있는 걸 쓴다.
            {
                UpdateStats((o, o1) => o1, modifier);
            }
            else if (modifier.statsChangeType == StatsChangeType.Add) // 2개의 값을 더한다.
            {
                UpdateStats((o, o1) => o + o1, modifier);
            }
            else if (modifier.statsChangeType == StatsChangeType.Multiple) // 2개의 값을 곱한다.
            {
                UpdateStats((o, o1) => o * o1, modifier);
            }
        }
    ...
    private void UpdateStats(Func<float, float, float> operation, CharacterStats newModifier)
    {
        ...
        {
            case RangedAttackData _: // 패턴 사용, 버려지는 매개변수 _, melee가 추가되면, 밑에case로 추가.
